...
def create_profile_view():
    try:
        ...
        cursor.execute("""
        CREATE OR REPLACE VIEW ewaste_user_profile_view AS
        SELECT 
            u.user_id,
            u.full_name, 
            u.email, 
            ...
            u.pincode,
            l.location_name,
            COALESCE(ep.reward_points, 0) AS ewaste_points
        FROM 
            ewaste_users u
        JOIN 
            locations l ON u.pincode = l.pincode
        LEFT JOIN 
            ewaste_points ep ON u.user_id = ep.user_id
        """)
        
        connection.commit()
        print("Profile view created successfully")
...
if __name__ == "__main__":
    ...
    user_email = "nn@gmail.com"
    profile_data = get_profile(user_email)
    
    if profile_data:
        print("\neWaste Management Profile")
        print("------------------------")
        print(f"Full Name: {profile_data['full_name']}")
        print(f"Email: {profile_data['email']}")
        print(f"Mobile: {profile_data['mobile']}")
        print(f"Address: {profile_data['address']}")
        print(f"Pincode: {profile_data['pincode']}")
        print(f"Location: {profile_data['location']}")
        print(f"Reward Points: {profile_data['ewaste_points']}")
