...
try:
    ...
    create_procedure = """
    CREATE OR REPLACE PROCEDURE solve_complaint(p_user_id IN NUMBER) AS
    BEGIN
        UPDATE complaints
        SET status = 'Solved'
    ...
    create_trigger = """
    CREATE OR REPLACE TRIGGER log_solved_complaint
    AFTER UPDATE ON complaints
    FOR EACH ROW
    WHEN (NEW.status = 'Solved')
    BEGIN
        INSERT INTO complaint_logs (log_id, user_id, change_date, action)
        VALUES (complaint_logs_seq.NEXTVAL, :NEW.user_id, SYSDATE, 'Status changed to Solved');
    END;
    """
    cursor.execute(create_trigger)

    user_id = 3
    cursor.callproc('solve_complaint', [user_id])
    print(f"Complaint with User ID {user_id} marked as Solved via procedure.")
