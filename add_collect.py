...
def register_collector(name, email, address, password):
    try:
        ...
        cursor.callproc("register_collector", [
            name,
            email,
            address,
            password,
        ...
        if status.getvalue() == 'SUCCESS':
            ...
            print(f"Registration failed: {error_msg.getvalue()}")
    ...
    register_collector(name, email, address, password)
